...
class TestFeatureFlags:
    ...
    def test_get_server_features_returns_deep_copy(self):
        """Test that nested feature flags are isolated from the global defaults."""
        features = get_server_features()
        features["extension"]["manager"]["supports_v4"] = False

        assert SERVER_FEATURE_FLAGS["extension"]["manager"]["supports_v4"] is True
    ...
    def test_get_connection_feature_with_invalid_feature_flags_type(self):
        """Test malformed feature flags payloads fail closed instead of crashing."""
        sockets_metadata = {
            "sid1": {"feature_flags": ["not", "a", "dict"]}
        }

        assert get_connection_feature(sockets_metadata, "sid1", "any_feature") is False
        assert get_connection_feature(
            sockets_metadata, "sid1", "any_feature", default="fallback"
        ) == "fallback"
