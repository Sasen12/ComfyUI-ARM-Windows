                ...
                with torch.inference_mode():
                    ...
                    while not execution_list.is_empty():
                        ...
                        if error is not None:
                            if not cpu_retry_used and comfy.model_management.should_retry_on_cpu_after_oom(ex):
                                cpu_retry_used = True
                                retry_on_cpu = True
                                logging.warning("DirectML OOM encountered; retrying the prompt on CPU.")
                                comfy.model_management.unload_all_models()
                                comfy.model_management.switch_to_cpu_mode(ex)
                                comfy.model_management.cleanup_models_gc()
                                break
